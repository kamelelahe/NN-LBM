...
//I decided to omit lz for simplicity.             in the casse of 3D simulation; add it here as same as lx
namespace plb {
...
public:
    ...
    CarreauFlowParam(T latticeU_, T Re_, T Cu_, T nuInf_, T n_,
                          plint resolution_,T timeDepend_, T lx_, T ly_, T lz_=T() )
        : latticeU(latticeU_), Re(Re_), Cu(Cu_), nuInf(nuInf_), n(n_),
          resolution(resolution_), timeDepend(timeDepend_), lx(lx_), ly(ly_), lz(lz_)
    { }
    ...
    /// The time dependancy factor
    T getTDfactor() const      { return timeDepend; }
    /// resolution
                  ...
                  std::string const& title)
{
   ...
   // plb_ofstream ofile(fullName.c_str(),  std::ofstream::app : std::ofstream::out);
    ofile << title << "\n\n";
    ...
    ofile << "time dependancy factor:                    TDfactor=" << parameters.getTDfactor() << "\n";
}
